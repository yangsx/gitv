<script lang="ts">
	import type { DiffLine, Hunk } from '$lib/bindings/types';
	...
	function lineKind(dl: DiffLine): 'context' | 'addition' | 'deletion' {
...
<div class="font-mono text-xs leading-5">
	{#each hunks as hunk}
		...
		{#each hunk.lines as line}
			...
			{@const bgClass = kind === 'addition' ? 'bg-green-900/30' : kind === 'deletion' ? 'bg-red-900/30' : ''}
			<div class="flex {bgClass}">
				...
				>
					{#if kind === 'addition'}+{:else if kind === 'deletion'}-{:else}&nbsp;{/if}
				</span>
