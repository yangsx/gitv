...
use crate::graph::layout::{GraphLayout, edge_segments};
        ...
        for seg in edge_segments(edge) {
            for w in seg.windows(2) {
                ...
                for nr in (r_lo + 1)..r_hi {
                    if nr == edge.from_row || nr == edge.to_row {
                        continue;
                    }
    ...
    for edge in &layout.edges {
        for seg in edge_segments(edge) {
            for w in seg.windows(2) {
                ...
                // Any single-row segment is a chamfer (horizontal-first or
                // vertical-first) rendered by the frontend — angle constraint
                // only applies to multi-row diagonal runs (must be 45°).
                if ddr == 1 {
                    continue;
                }
                if ddr != ddc {
                    result.push(format!(
                        "edge ({},{})\u{2192}({},{}) segment ({},{})\u{2192}({},{}) has bad angle (|dr|={}, |dc|={})",
                        edge.from_row, edge.from_col, edge.to_row, edge.to_col,
                        r1, c1, r2, c2, ddr, ddc,
                    ));
                }
...
/// Single-row segments (chamfers) are completely exempt.
    ...
    for edge in &layout.edges {
        for seg in edge_segments(edge) {
            for w in seg.windows(2) {
                ...
                // Any single-row segment is a chamfer — no continuity constraint.
                if row_delta == 1 {
                    continue;
                }
                if col_delta > row_delta {
                    result.push(format!(
                        "edge ({},{})\u{2192}({},{}) segment ({},{})\u{2192}({},{}) moves {} cols over {} rows (max {})",
                        edge.from_row, edge.from_col, edge.to_row, edge.to_col,
...
///
/// Chamfer exemption: a zigzag involving a single-row segment (chamfer) is not
/// a "consecutive-row zigzag" — the invariant targets direction reversals that
/// span multiple rows. Single-row segments are exempt from `thread_continuity`
/// (invariant #7) for the same reason: the frontend renders them as
/// horizontal-first diagonals that can span any number of columns.
pub fn check_no_zigzag(layout: &GraphLayout) -> PropertyResult {
    ...
    for edge in &layout.edges {
        ...
        for w in path.windows(3) {
            ...
            // Chamfer exemption: skip if either segment is a single-row
            // transition. Consistent with `check_thread_continuity` which
            // exempts single-row segments from the ≤1 col/row rule.
